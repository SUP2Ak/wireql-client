...
exports.queryTypesExample = exports.advancedExample = exports.basicExample = void 0;
const src_1 = require("../src");
async function basicExample() {
    console.log('🚀 Exemple d\'utilisation WireQL\n');
    // Création du client
    const client = (0, src_1.createSimpleClient)({
        host: 'localhost',
        port: 8080,
        apiKey: 'admin-key',
    ...
    try {
        ...
        const activeUsers = await client.query('SELECT * FROM users WHERE active = ? AND level > ?', [true, 10]);
...
async function advancedExample() {
    console.log('\n🔧 Exemple avancé avec configuration complète\n');
    const advancedClient = (0, src_1.createSimpleClient)({
...
async function queryTypesExample() {
    ...
    try {
        ...
        const transactionSteps = [
            { op: src_1.SqlOperation.Insert, query: 'INSERT INTO transactions (amount) VALUES (?)', values: [100] },
            { op: src_1.SqlOperation.Update, query: 'UPDATE wallets SET balance = balance + ? WHERE user_id = ?', values: [100, 456] }
        ];
...
exports.queryTypesExample = queryTypesExample;
